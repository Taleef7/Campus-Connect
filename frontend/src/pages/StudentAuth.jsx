// StudentAuth.jsx (Fully Polished Version)
...
const StudentAuth = ({ isSignup, embedded = false }) => {
    ...
    try {
      ...
      if (!userSnap.exists()) {
        await setDoc(userRef, {
          name, email, major: '', role: 'student', createdAt: new Date().toISOString(),
        });
        navigate('/student-dashboard');
      } else {
        if (userSnap.data().role === 'student') {
          ...
          await signOut(auth);
          setErrorMsg('Access denied. Not a student account.');
        }
      }
    } catch (error) {
